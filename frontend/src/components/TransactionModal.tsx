...
export default function TransactionModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingTransaction) {
        ...
        if (response.data.newly_unlocked_achievements?.length > 0) {
      ...
      setFormData({
        ...
        spent_at_time: new Date().toTimeString().slice(0, 5),
        currency: 'USD',
      });
  ...
  const handleAchievementPopupClose = async () => {
  setNewAchievements([]);
  try {
    await api.post('/achievements/mark-seen');
  } catch (error) {
    console.error('Failed to mark achievements:', error);
  }
};
